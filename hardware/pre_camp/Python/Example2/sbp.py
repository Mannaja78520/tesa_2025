		...
		for i in range(2, self.row):
			for j in range(1, self.col - 1):
				if blockNumber == tempState.getMatrix()[i][j]:
					if direction == "up":
						self.matrix[i-1][j] = tempState.getMatrix()[i][j]
						self.matrix[i][j] = 0
					elif direction == "down":
						self.matrix[i+1][j] = tempState.getMatrix()[i][j]
						self.matrix[i][j] = 0
					elif direction == "left":
						self.matrix[i][j-1] = tempState.getMatrix()[i][j]
						self.matrix[i][j] = 0
...
def calculateH(state: BoardState):
	matrix = state.getMatrix()
	row = matrix[0][1]
	col = matrix[0][0]
	goalLocation = []
	...
	for i in range(1, row):
		for j in range(col):
			if (matrix[i][j] == -1) & (len(goalLocation) == 0):
				goalLocation.append(i)
				goalLocation.append(j)
				break
			if (matrix[i][j] == 2) & (len(currentLocation) == 0):
				currentLocation.append(i)
				currentLocation.append(j)
				break
		if (len(goalLocation) == 2) & (len(currentLocation) == 2):
			break
	if (len(goalLocation) == 0) & (len(currentLocation) == 2):
		return 0
	else:
		return math.sqrt( (goalLocation[0] - currentLocation[0])**2 + (goalLocation[1] - currentLocation[1])**2 )

class Node:
	def __init__(self, state: BoardState, parent: BoardState, cost: int):
		self.state = state
		self.parent = parent
		self.previousMove = ""
		self.heuristic = calculateH(self.state)
		self.cost = cost
	def getCost(self):
		return self.cost
	...
	def getChildren(self):
		children = []
		for i in self.state.availableMoves():
			child = self.clone()
			child.setCost(child.getCost() + 1)
			child.setParent(self)
			child.setPreviousMove(i)
			moveData = i.replace("(", "").replace(")", "").split(",")
			childState = child.getState()
			childState.move(int(moveData[0]), moveData[1])
	...
	def getMoveList(self):
		...
		for i in self.availableMoves():
			moveData = i.replace("(", "").replace(")", "").split(",")
			moveList.append(moveData)
		return moveList
	def clone(self):
		clone = Node(self.state.clone(), self.parent.clone(), self.cost)
		return clone

def getPath(goalNode: Node):
	path = [goalNode]
	parentNode = goalNode.getParent()
	while parentNode.getPreviousMove() != "":
...
def bfs(state: BoardState):
	exploredNodes = 0
	openList = [Node(state, state, 0)]
	closedList = []
	startTime = time.time()
	while len(openList) > 0:
		...
		if (node.getState().isDone()):
			...
			return [getPath(node), node, exploredNodes, finishTime - startTime]
		closedList.append(node)
		children = node.getChildren()
		for i in children:
			notInOpenList = True
			for j in openList:
				if compareStates(i.getState(), j.getState()):
					notInOpenList = False
					break
...
def dfs(state: BoardState):
	...
	while len(openList) > 0:
		...
		notInClosedList = True
		for i in closedList:
			if compareStates(node.getState(), i.getState()):
				notInClosedList = False
				break
		if notInClosedList:
			exploredNodes += 1
...
def astar(state: BoardState):
	...
	while len(openList) > 0:
		...
		for i in range(len(openList)):
			if openList[i].getF() < openList[indexToPop].getF():
				indexToPop = i
		node = openList.pop(indexToPop)
		exploredNodes += 1
		if (node.getState().isDone()):
			finishTime = time.time()
			return [getPath(node), node, exploredNodes, finishTime - startTime]
...
if (command == "print"):
	...
	[path, goal, exploredNodes, exeTime] = bfs(board.getState(0))
	for i in range(1, len(path)):
		print(path[i].getPreviousMove())
	print("")
	printMatrix(goal.getState().getMatrix())
	print("")
	print(exploredNodes)
	print(round(exeTime, 2))
	print(len(path) - 1)
elif (command == "dfs"):
	[path, goal, exploredNodes, exeTime] = dfs(board.getState(0))
	for i in range(1, len(path)):
		print(path[i].getPreviousMove())
	print("")
	printMatrix(goal.getState().getMatrix())
	...
	[path, goal, exploredNodes, exeTime] = ids(board.getState(0))
	for i in range(1, len(path)):
		print(path[i].getPreviousMove())
	print("")
	printMatrix(goal.getState().getMatrix())
	...
	[path, goal, exploredNodes, exeTime] = astar(board.getState(0))
	for i in range(1, len(path)):
		print(path[i].getPreviousMove())
	print("")
	printMatrix(goal.getState().getMatrix())
