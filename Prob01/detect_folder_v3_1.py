# detect_folder_v3_1.py
# Final Drone Detector v3.1 (Cloud-robust)
# - Auto-sky mask + CLAHE
# - Auto pole removal (Hough)
# - OSD masking
# - Black-hat + EDGE GATE (สำคัญสำหรับกันเมฆ)
# - ROI proposal (adaptive สำหรับฟ้าครึ้ม)
# - Shape + Texture Filters (variance/edges/high-freq/entropy/lines)
...
BH_FOLDER      = 'RESULTS_BLACKHAT'          # เซฟ BH ของทุกรูป
...
# -------- ROI proposal (cloud-robust) --------
def propose_roi_from_blackhat(enh, sky_mask=None, min_area=140, pad=16):
    H, W = enh.shape
    work = enh.copy()
    ...
    perc = 99.1 if mean_val < 80 else 98.7  # ผ่อน threshold ให้ตรวจวัตถุอ่อนขึ้น
    ...
    for c in cnts:
        ...
        if not (0.5 <= aspect <= 3.0): continue
...
def is_cloud_like(region):
    ...
    if var < 45: return True   # เดิม 55

    edges = cv2.Canny(region, 60, 150)
    edge_density = np.count_nonzero(edges) / (region.size + 1e-6)
    if edge_density < 0.025: return True  # เดิม 0.035

    lap = cv2.Laplacian(region, cv2.CV_32F, ksize=3)
    hf_ratio = (np.abs(lap) > 8).mean()
    if hf_ratio < 0.010: return True  # เดิม 0.015

    ent = _entropy(region)
    if ent < 4.4: return True  # เดิม 4.6

    lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=8,
                            minLineLength=max(5, int(min(region.shape)*0.20)),
                            maxLineGap=3)
    if lines is None:
        return False  # << เปลี่ยน! ถ้าไม่เจอเส้น ไม่ตัดทิ้งอัตโนมัติแล้ว
    return False
...
def process_image(path):
    ...
    for (rx, ry, rw, rh) in rois:
        ...
        for c in cnts:
            ...
            if area < 15 or area > 9000: continue
            x,y,w,h = cv2.boundingRect(c)
    ...
    return vis, final_boxes, enh_raw, enh   # ส่ง BH ทั้งสองแบบ
